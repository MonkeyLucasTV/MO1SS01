...
int IRServeurUDP::RecevoirUnMessage(std::string & message)
{
	...
	int nbCaracteresRecus = recvfrom(m_socketServeurUDP, msg, 1500, 0, &from,(int *)(&length));
...
int IRServeurUDP::RecevoirDesOctets(char * tableau, int tailleMax)
{
    ...
    int nbCaracteresRecus = recvfrom(m_socketServeurUDP, tableau, tailleMax, 0, &from, (int *)(&length));
    if(nbCaracteresRecus > 1)
    {
        return nbCaracteresRecus;
    }
